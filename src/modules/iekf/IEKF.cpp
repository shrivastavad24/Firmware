	...
	_timestampGps()
{
	...
	_P(Xe::rot_n, Xe::rot_n) = 5;
	_P(Xe::rot_e, Xe::rot_e) = 5;
	_P(Xe::rot_d, Xe::rot_d) = 10;
	_P(Xe::vel_n, Xe::vel_n) = 1e6;
	_P(Xe::vel_e, Xe::vel_e) = 1e6;
	_P(Xe::vel_d, Xe::vel_d) = 1e6;
	_P(Xe::gyro_bias_n, Xe::gyro_bias_n) = 1e-3;
	_P(Xe::gyro_bias_e, Xe::gyro_bias_e) = 1e-3;
	_P(Xe::gyro_bias_d, Xe::gyro_bias_d) = 1e-3;
	_P(Xe::accel_scale, Xe::accel_scale) = 1e-3;
	_P(Xe::pos_n, Xe::pos_n) = 1e6;
	_P(Xe::pos_e, Xe::pos_e) = 1e6;
	_P(Xe::pos_d, Xe::pos_d) = 1e6;
	_P(Xe::terrain_alt, Xe::terrain_alt) = 1e6;
	_P(Xe::baro_bias, Xe::baro_bias) = 1e6;
...
void IEKF::correctAccel(const sensor_combined_s *msg)
{
	...
	float relNormError = (Vector3<float>(y_b / _x(X::accel_scale)).norm()
			      - _g_n.norm()) / _g_n.norm();
...
void IEKF::correctGps(const vehicle_gps_position_s *msg)
{
	...
	R(Y_gps::pos_n, Y_gps::pos_n) = 0.05f / dt;
	R(Y_gps::pos_e, Y_gps::pos_e) = 0.05f / dt;
	R(Y_gps::pos_d, Y_gps::pos_d) = 0.05f / dt;
	...
	for (int i = 0; i < Xe::n; i++) {
		...
		for (int j = 0; j <= i; j++) {
			if (!PX4_ISFINITE(_P(i, j))) {
				ROS_DEBUG("P(%d, %d) NaN, setting to 0", i, j);
				_P(i, j) = 0;
			}
...
void IEKF::boundX()
{
	...
	for (int i = 0; i < X::n; i++) {
		if (!PX4_ISFINITE(_x(i))) {
			ROS_WARN("x(%d) NaN, setting to 0", i);
			_x(i) = 0.0;
		}
...
void IEKF::publish()
{
	...
	if (v_stddev < 1.0f && _origin.xyInitialized() && _origin.altInitialized()) {
		double lat_deg = 0;
		double lon_deg = 0;
		float alt_m = 0;
